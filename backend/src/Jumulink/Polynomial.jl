abstract type AbstractFraction end

"""
多项式结构体,储存多项式系数向量,按幂次升序.
"""
Base.@kwdef struct Polynomial <: AbstractFraction
...
定义分式
"""
struct Fraction <: AbstractFraction
...
Base.:/(x::Number, y::Fraction) = Fraction(x * y.denominator, y.numerator)
Base.:/(x::Fraction, y::Number) = Fraction(x.numerator, y * x.denominator)
...
function Base.:*(x::Fraction, y::Fraction)
    ...
    if x1 isa Fraction && y1 isa Fraction
