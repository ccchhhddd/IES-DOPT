<template>
  ...
  <n-space vertical>
    ...
    <n-card :bordered="false" class="rounded-16px shadow-sm">
      <p class="text-16px font-bold inline-block">优化项目选择</p>
      <p class="text-16px text-red inline-block">*</p>
      <n-checkbox-group v-model:value="items">
        <n-space item-style="display: flex;">
          <n-checkbox value="1" label="风力装机容量" />
          <n-checkbox value="2" label="光伏装机容量" />
          <n-checkbox value="3" label="燃气轮机装机容量" />
        </n-space>
      </n-checkbox-group>
    </n-card>
    <n-card class="rounded-16px shadow-sm">
...
function simulateToServer() {
  ...
  const optlist = [0, 0, 0, 0, 0, 0, 0, 0];
  console.log(items.value);
  if (items.value) {
    for (const [key, value] of Object.entries(items.value)) {
      optlist[Number(value) - 1] = 1;
    }
  }
  request
