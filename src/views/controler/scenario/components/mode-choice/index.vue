<template>
  <n-divider title-placement="center">{{ props.title + props.description }}PID控制实验</n-divider>
    ...
    <n-grid-item span="0:24 640:24 1024:12">
      <n-card :bordered="false" class="rounded-16px shadow-sm">
        <n-space justify="center">
          <p class="text-28px font-bold pb-12px">系统原理图</p>
        </n-space>
        <n-space v-if="modeChoosed == 1" justify="center">
          <n-image src="/PID.jpg" alt="mode-choice" width="600" />
        </n-space>
              ...
              <n-collapse :accordion="true">
                <n-collapse-item v-for="(val, key, ind) in simulationParamsInput" :title='key' :name='ind'>
                  ...
                  <n-space vertical justify='space-between' size='large' style='margin-bottom: 10px;'>
                    <n-input v-for="(val_input, key_input, _) in (Object.fromEntries(Object.entries(val).filter(([key,_])=>key!=='')) as { [key: string]: number })"
  ...
  <n-space vertical>
    <n-card :bordered='false' class='rounded-16px shadow-sm'>
      <n-space justify='center'>
        <p class='text-24px font-bold pb-12px'>理想PID控制响应图</p>
      ...
      <div ref='lineRef' class='w-full h-640px' style="margin-top: 15px;"></div>
		...
		<n-card :bordered='false' class='rounded-16px shadow-sm'>
      <n-space justify='center'>
        <p class='text-24px font-bold pb-12px'>理想PID扰动响应图</p>
        <!-- <n-switch v-model:value="dayOrWeek" size="large" class='pt-15px' @update:value="updateSwich">
          <template #checked> 周数据图 </template>
          <template #unchecked> 日数据图 </template>
        </n-switch> -->
      ...
      <div ref='lineRef1' class='w-full h-640px' style="margin-top: 15px;"></div>
		...
		<n-card :bordered='false' class='rounded-16px shadow-sm'>
      <n-space justify='center'>
        <p class='text-24px font-bold pb-12px'>实际PID控制响应图</p>
        <!-- <n-switch v-model:value="dayOrWeek" size="large" class='pt-15px' @update:value="updateSwich">
          <template #checked> 周数据图 </template>
          <template #unchecked> 日数据图 </template>
        </n-switch> -->
      ...
      <!-- <n-slider v-if="!dayOrWeek" v-model:value="dayChoiceSlider" :step="1" :max="365" :min="1"
        :on-update:value="updateFigure" />
      <n-slider v-else v-model:value="dayChoiceSlider" :step="1" :max="52" :min="1" :on-update:value="updateFigure" /> -->
      <div ref='lineRef2' class='w-full h-640px' style="margin-top: 15px;"></div>
    </n-card>

		<n-card :bordered='false' class='rounded-16px shadow-sm'>
      <n-space justify='center'>
        <p class='text-24px font-bold pb-12px'>实际PID扰动响应图</p>
        <!-- <n-switch v-model:value="dayOrWeek" size="large" class='pt-15px' @update:value="updateSwich">
          <template #checked> 周数据图 </template>
          <template #unchecked> 日数据图 </template>
        </n-switch> -->
      ...
      <!-- <n-slider v-if="!dayOrWeek" v-model:value="dayChoiceSlider" :step="1" :max="365" :min="1"
        :on-update:value="updateFigure" />
      <n-slider v-else v-model:value="dayChoiceSlider" :step="1" :max="52" :min="1" :on-update:value="updateFigure" /> -->
      <div ref='lineRef3' class='w-full h-640px' style="margin-top: 15px;"></div>
    </n-card>
  </n-space>
</template>
...
const lineOptions3 = ref<ECOption>({
  //  显示工具箱
  toolbox: {
    show: true,
    orient: 'vertical',
...
const { domRef: lineRef3 } = useEcharts(lineOptions3);
...
type FigureData = {
  xyAxis: Array<number>
	...
	xyAxis2: Array<number>
	xyAxis3: Array<number>
}
...
const figureData = ref<FigureData>({ xyAxis: [], xyAxis1: [],xyAxis2: [],xyAxis3: []}) // 图数据
...
const updateFigure = () => {
  ...
  lineOptions2.value.series = Object.keys(figureData.value.xyAxis2).map((key) => {
    return {
      name: key,
      type: 'line',
      smooth: true,
      ...
      data: figureData.value.xyAxis2
    }
  })
  lineOptions2.value.legend = {
    data: []
  }

	//图4画面
  lineOptions3.value.yAxis = {
    type: 'value',
    name: '响应值',
    axisPointer: {
      snap: true
  ...
  lineOptions3.value.series = Object.keys(figureData.value.xyAxis3).map((key) => {
    return {
      name: key,
      type: 'line',
      smooth: true,
      ...
      data: figureData.value.xyAxis3
    }
  })
  lineOptions3.value.legend = {
    data: []
  }
}
