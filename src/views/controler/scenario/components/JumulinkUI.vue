...
import ZeroPole from './ZeroPole.vue';
...
function onRestore(data: any) {
  const reader = new FileReader();
  reader.readAsText(data.file.file);
  reader.onload = res => {
    if (res) {
      const flow = JSON.parse(res.target.result as string);
      fromObject(flow.edges);
      simArgs.value.nodes = new Map(Object.entries(flow.nodes));
      id = flow.num;
    }
  };
}
          ...
          <n-grid :cols="2" :y-gap="20" style="width: 300px; justify-items: center; align-items: center">
            ...
            <n-grid-item>
              <div
                ...
                :draggable="true"
                style="display: flex; justify-content: center; align-items: center"
                @dragstart="onDragStart($event, 'zeroPole')"
              >
                <p>Zero-Pole</p>
              </div>
              <div style="text-align: center">零极点式传函</div>
            </n-grid-item>
          </n-grid>
        </n-scrollbar>
